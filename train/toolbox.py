...
# 读取训练集和开发集中的文本信息：字符、标签、ngram、拼音、五笔，然后写入相关文件供后续使用
# def get_text_information(path, filelist, ngram=1):
#     out_char = codecs.open(path + '/pre_vocab.txt', 'w', encoding='utf-8')
...
#
#
def read_text_information(path, ngrams=1):
    chars = []
    tag_set = {}
    ngram_set = None
    for line in codecs.open(path + '/pre_vocab.txt', 'rb', encoding='utf-8'):
...
def read_sample_embedding(path, emb_file_name):
    emb_values = []
    f = codecs.open(path + '/' + emb_file_name + '.txt', 'rb', encoding='utf-8')
    vec = f.readline()
    nums = int(vec.split(" ")[0])
    for line in f:
        line = line.strip()
        sets = line.split(' ')
        emb_values.append(np.asarray(sets[1:], dtype='float32'))
    emb_dim = len(emb_values[0])
    return nums, emb_dim, emb_values


def read_ngram_embedding(path, ngram_emb_file_name, n):
    embs = []
    for i in range(2, n + 1):
        _, _, emb_v = read_sample_embedding(path, ngram_emb_file_name + '_' + str(i) + 'gram')
        embs.append(emb_v)
    return embs
...
def get_radical_dic(path='radical.txt'):
    ...
    读取偏旁部首字典
    :param path: 字典文件路径
    :return: a python dictionary, key 是 str 码，value 是字符
    """
    rad_dic = {}
...
def get_radical_idx(ch, rad_dic, keys=None):
    """
    给定一个汉字，从偏旁部首字典中找到对应部首的 str 码
    基本思想是，汉字的 str 编码是按照偏旁部首顺序来编码的，
    仔细查看汉字的 str 码表即可找出规律
    :param ch:  目标汉字
    :param rad_dic:  部首字典，key=str 码，value=部首字符
...
def get_new_pinyin_embeddings(new_chars, pinyin_emb_dim, pinyin_emb_path=None):
    if pinyin_emb_path is None:
        return tf.random_uniform([len(new_chars), pinyin_emb_dim], -math.sqrt(3 / pinyin_emb_dim), math.sqrt(3 / pinyin_emb_dim))
    else:
        assert os.path.isfile(pinyin_emb_path)
        pinyin_emb = {}
        new_pinyin_emb = []
        for line in codecs.open(pinyin_emb_path, 'rb', encoding='utf-8'):
            line = line.strip()
            sets = line.split(' ')
            pinyin_emb[sets[0]] = np.asarray(sets[1:], dtype='float32')
        if '<UNK>' not in pinyin_emb:
            unk = np.random.uniform(-math.sqrt(float(3) / pinyin_emb_dim), math.sqrt(float(3) / pinyin_emb_dim), pinyin_emb_dim)
            pinyin_emb['<UNK>'] = np.asarray(unk, dtype='float32')
        for ch in new_chars:
            if lazy_pinyin(ch) in pinyin_emb:
                new_pinyin_emb.append(pinyin_emb[ch])
            else:
                new_pinyin_emb.append(pinyin_emb['<UNK>'])
        return tf.stack(new_pinyin_emb)

def get_new_wubi_embeddings(new_chars, wubi_emb_dim, wubi_emb_path=None):
    ...
    if wubi_emb_path is None:
        return tf.random_uniform([len(new_chars), wubi_emb_dim], -math.sqrt(3 / wubi_emb_dim), math.sqrt(3 / wubi_emb_dim))
    else:
        assert os.path.isfile(wubi_emb_path)
        wubi_emb = {}
        new_wubi_emb = []
        for line in codecs.open(wubi_emb_path, 'rb', encoding='utf-8'):
            line = line.strip()
            sets = line.split(' ')
            wubi_emb[sets[0]] = np.asarray(sets[1:], dtype='float32')
        if '<UNK>' not in wubi_emb:
            unk = np.random.uniform(-math.sqrt(float(3) / wubi_emb_dim), math.sqrt(float(3) / wubi_emb_dim), wubi_emb_dim)
            wubi_emb['<UNK>'] = np.asarray(unk, dtype='float32')
        for ch in new_chars:
            wubi = None
            if dic_wubi.has_key(ch):
                wubi = dic_wubi[ch]
            else:
                wubi = dic_wubi['<UNK>']
            if wubi in wubi_emb:
                new_wubi_emb.append(wubi_emb[ch])
            else:
                new_wubi_emb.append(wubi_emb['<UNK>'])
        return tf.stack(new_wubi_emb)
...
def get_input_vec(path, filename, char2index, tag2index, pinyin_dic_file=None, wubi_dic_file=None, char2freq=None, tag_scheme='BIES'):
    # initialize vector
    max_sent_len_c = 0
    max_sent_len_w = 0
    max_word_len = 0
    t_len = 0
    x_m = [[]]
    if pinyin_dic_file is not None:
        x_m.append([])
    if wubi_dic_file is not None:
        x_m.append([])
    ...
    for line in codecs.open(path + '/' + filename, 'r', encoding='utf-8'):
        char_indices = []
        raw_l = ''
        if pinyin_dic_file is not None:
            pinyin_indices = []
        ...
        if len(segs) > 0 and len(line) > 0:
            for seg in segs:
                ...
                if w_len == 1:
                    ...
                    char_indices.append(char2index[splits[0]])

                    if pinyin_dic_file is not None:
                        pinyin = lazy_pinyin(splits[0].decode('utf-8'))[0].encode('utf-8')
                        if pinyin.decode('utf-8') in pinyin_dic:
                            pinyin_indices.append(pinyin_dic[pinyin.decode('utf-8')])
                        else:
                            pinyin_indices.append(pinyin_dic['<UNK>'])

                    if wubi_dic_file is not None:
                        if dic_wubi.has_key(splits[0].encode('utf8')):
                            if dic_wubi[splits[0].encode('utf-8')] in wubi_dic:
                                wubi_indices.append(wubi_dic[dic_wubi[splits[0].encode('utf-8')]])
                            else:
                                wubi_indices.append(wubi_dic['<UNK>'])
                        else:
                            if splits[0].encode('utf-8') in wubi_dic:
                                wubi_indices.append(wubi_dic[splits[0].encode('utf-8')])
                            else:
                                wubi_indices.append(wubi_dic['<UNK>'])
                    ...
                    tag_indices['seg'].append(tag2index['seg']['S'])
                    tag_indices['BI'].append(tag2index['BI']['B-' + splits[1]])
                    tag_indices['BIE'].append(tag2index['BIE']['B-' + splits[1]])
                    tag_indices['BIES'].append(tag2index['BIES']['S-' + splits[1]])

                else:

                    for x in range(w_len):
                        c_ch = splits[0][x]
                        char_indices.append(char2index[c_ch])

                        if pinyin_dic_file is not None:
                            pinyin = lazy_pinyin(c_ch.decode('utf-8'))[0].encode('utf-8')
                            if pinyin.decode('utf-8') in pinyin_dic:
                                pinyin_indices.append(pinyin_dic[pinyin.decode('utf-8')])
                            else:
                                pinyin_indices.append(pinyin_dic['<UNK>'])
                        ...
                        if wubi_dic_file is not None:
                            if dic_wubi.has_key(c_ch.encode('utf-8')):
                                if dic_wubi[c_ch.encode('utf-8')] in wubi_dic:
                                    wubi_indices.append(wubi_dic[dic_wubi[c_ch.encode('utf-8')]])
                                else:
                                    wubi_indices.append(wubi_dic['<UNK>'])
                            else:
                                if c_ch.encode('utf-8') in wubi_dic:
                                    wubi_indices.append(wubi_dic[c_ch.encode('utf-8')])
                                else:
                                    wubi_indices.append(wubi_dic['<UNK>'])
                        ...
                        if x == 0:
                            ...
                            tag_indices['seg'].append(tag2index['seg']['B'])

                            tag_indices['BI'].append(tag2index['BI']['B-' + splits[1]])
                            tag_indices['BIE'].append(tag2index['BIE']['B-' + splits[1]])
                            tag_indices['BIES'].append(tag2index['BIES']['B-' + splits[1]])

                        elif x == len(splits[0]) - 1:

                            tag_indices['seg'].append(tag2index['seg']['E'])

                            tag_indices['BI'].append(tag2index['BI']['I-' + splits[1]])
                            tag_indices['BIE'].append(tag2index['BIE']['E-' + splits[1]])
                            tag_indices['BIES'].append(tag2index['BIES']['E-' + splits[1]])

                        else:

                            tag_indices['seg'].append(tag2index['seg']['I'])

                            tag_indices['BI'].append(tag2index['BI']['I-' + splits[1]])
                            tag_indices['BIE'].append(tag2index['BIE']['I-' + splits[1]])
                            tag_indices['BIES'].append(tag2index['BIES']['I-' + splits[1]])
            ...
            # -----------------------------
            i = 1  # i is just for append
            x_m[0].append(char_indices)
            if char2freq is not None:
                x_m[-i].append(freqs)
                i += 1
            if wubi_dic_file is not None:
                x_m[-i].append(wubi_indices)
                i += 1
            if pinyin_dic_file is not None:
                x_m[-i].append(pinyin_indices)

            y_m[0].append(tag_indices[tag_scheme])
...
def generate_output(chars, tags, ner_tags, tag_scheme):
    out = []
            ...
            for ch, tg in zip(chs, tgs):
                if tag_scheme == 'seg':
                    ...
                    if tg_sets[0] == 'I' and tg_sets[1] == c_tag:
                        ...
                        if c_tag in ner_tags:
                            q_line += ' ' + c_word + '_' + c_tag
                        else:
                            q_line += ' ' + c_word + '_' + 'O'
                        c_word = ch
                        if len(tg_sets) < 2:
            ...
            if len(c_word) > 0:
                if tag_scheme == 'seg':
                    ...
                    if c_tag in ner_tags:
                        ...
                        q_line += ' ' + c_word + '_' + 'O'
            if tag_scheme == 'seg':
                sub_out_pos.append(p_line[8:])
                sub_out_ner.append(q_line[8:])
            else:
                sub_out_pos.append(p_line[3:])
                sub_out_ner.append(q_line[3:])
        out.append(sub_out_pos)
        out.append(sub_out_ner)
    return out


def evaluator(prediction, gold, metric='F1-score', tag_num=1, verbose=False):
    assert len(prediction) == len(gold)
    scores_pos = (0, 0, 0, 0, 0, 0)
    scores_ner = (0, 0, 0, 0, 0, 0)
    scores_b_pos = (0, 0, 0, 0, 0, 0)
    scores_b_ner = (0, 0, 0, 0, 0, 0)
    if metric in ['F1-score', 'Precision', 'Recall', 'All']:
        scores_pos = score(gold[0], prediction[0], tag_num, verbose)
        scores_ner = score(gold[1], prediction[1], tag_num, verbose)
    if metric in ['Boundary-F1-score', 'All']:
        scores_b_pos = score_boundaries(gold[0], prediction[0], verbose)
        scores_b_ner = score_boundaries(gold[1], prediction[1], verbose)
    return [scores_pos + scores_b_pos, scores_ner + scores_b_ner]
