...
def dot_product_attention(q, k, v,
    ...
    with tf.variable_scope("dot_product_attention", values=[q, k, v]):
        ...
        if special_attentions:
            logits = tf.concat([logits] + list(map(lambda x: tf.expand_dims(x, 1), special_attentions)), axis=1)
...
def multihead_attention(antecedent,
    ...
    with tf.variable_scope("multihead_attention", values=[antecedent]):
        ...
        num_basic_attention_heads = num_heads - len(special_attentions)
        ...
        q *= head_size ** -0.5
        x, attn_weights = dot_product_attention(q, k, v, bias,  special_attentions, dropout_rate)
        x = combine_heads(x)
...
def transformer(inputs, seq_lengths, head_size, num_heads, attn_dropout, relu_dropout, prepost_dropout,
                relu_hidden_size, special_attentions=[], layers=1, predict_label_embeddings=[]):
    # todo deal with special_attention, special_values

    if predict_label_embeddings:
        inputs_lists = [inputs]
        inputs_lists.extend(predict_label_embeddings)
        inputs = tf.concat(axis=2, values=inputs_lists)
        ...
        for i_layer in range(layers):
            with tf.variable_scope('layer_%d'%i_layer):
                ...
                with tf.variable_scope("self_attention"):
                    ...
                    y, attn_weights = multihead_attention(x, mask, num_heads, head_size, attn_dropout,
                                                          special_attentions)
